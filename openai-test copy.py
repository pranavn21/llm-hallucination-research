...
def runCoVe(questionList, startIndex, endIndex, GPTversion):
  index = 0
  userContent = "You are a virtual political assistant, skilled in providing expertise in United States (U.S.) politics and the history of U.S. politics, including U.S. politicians."
  for index in range(startIndex, endIndex):
    ...
    completion = client.chat.completions.create(
      model=GPTversion,
      messages=[
        {"role": "system", "content": questionList[index]},
    ...
    model=GPTversion,
    messages=[
       {"role": "system", "content": "The verification questions are: " + verificationResponse + "\n Generate responses for these questions."},
       {"role": "user", "content": userContent}
      ]
    )
